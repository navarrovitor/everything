# Movie 1
m = Movie.new
m.title = 'Avatar'
m.year = 2009
m.relevance = 8
m.save
# -------------------------
# Movie 2
m = Movie.new
m.title = 'Titanic'
m.year = 1997
m.relevance = 10
m.save
# -------------------------
# Movie 3
m = Movie.new
m.title = 'Star Wars Ep VII: The Force Awakens'
m.year = 2015
m.relevance = 7
m.save
# -------------------------
# Movie 4
m = Movie.new
m.title = 'Avengers: Infinity War'
m.year = 2018
m.relevance = 7
m.save
# -------------------------
# Movie 5
m = Movie.new
m.title = 'Jurassic World'
m.year = 2015
m.relevance = 7
m.save
# -------------------------
# Movie 6
m = Movie.new
m.title = 'The Avengers'
m.year = 2012
m.relevance = 7
m.save
# -------------------------
# Movie 7
m = Movie.new
m.title = 'The Shawshank Redemption'
m.year = 1994
m.relevance = 9
m.save
# -------------------------
# Movie 8
m = Movie.new
m.title = 'The Godfather'
m.year = 1972
m.relevance = 10
m.save
# -------------------------
# Movie 9
m = Movie.new
m.title = 'The Godfather: Part II'
m.year = 1974
m.relevance = 9
m.save
# -------------------------
# Movie 10
m = Movie.new
m.title = 'The Dark Knight'
m.year = 2008
m.relevance = 10
m.save
# -------------------------
# Movie 11
m = Movie.new
m.title = "Schindler's List"
m.year = 1993
m.relevance = 9
m.save
# -------------------------
# Movie 12
m = Movie.new
m.title = 'The Lord of the Rings: The Return of the King'
m.year = 2003
m.relevance = 9
m.save
# -------------------------
# Movie 13
m = Movie.new
m.title = "Pulp Fiction"
m.year = 1994
m.relevance = 10
m.save
# -------------------------
# Movie 14
m = Movie.new
m.title = 'Fight Club'
m.year = 1999
m.relevance = 9
m.save
# -------------------------
# Movie 15
m = Movie.new
m.title = "Lord of the Rings: the Fellowship of the Ring"
m.year = 2001
m.relevance = 10
m.save
# -------------------------
# Movie 16
m = Movie.new
m.title = 'Forrest Gump'
m.year = 1994
m.relevance = 9
m.save
# -------------------------
# Movie 18
m = Movie.new
m.title = 'The Matrix'
m.year = 1999
m.relevance = 10
m.save
# -------------------------
# Movie 19
m = Movie.new
m.title = "The Silence of the Lambs"
m.year = 1991
m.relevance = 9
m.save
# -------------------------
# Movie 20
m = Movie.new
m.title = 'Interstellar'
m.year = 2014
m.relevance = 9
m.save
# -------------------------
# # Movie 21
m = Movie.new
m.title = "Modern Times"
m.year = 1936
m.relevance = 7
m.save
# # -------------------------
# # Movie 22
m = Movie.new
m.title = 'Back to the Future'
m.year = 1985
m.relevance = 8
m.save
# # -------------------------
# # Movie 23
m = Movie.new
m.title = "Whiplash"
m.year = 2014
m.relevance = 7
m.save
# -------------------------
# Movie 24
m = Movie.new
m.title = 'Raiders of the Lost Ark'
m.year = 1981
m.relevance = 8
m.save
# -------------------------
# Movie 25
m = Movie.new
m.title = "The Lion King"
m.year = 1994
m.relevance = 9
m.save
# -------------------------
# Movie 26
m = Movie.new
m.title = 'Memento'
m.year = 2000
m.relevance = 6
m.save
# -------------------------
# Movie 27
m = Movie.new
m.title = "The Shining"
m.year = 1980
m.relevance = 7
m.save
# -------------------------
# Movie 28
m = Movie.new
m.title = 'Django Unchained'
m.year = 2012
m.relevance = 7
m.save
# -------------------------
# Movie 29
m = Movie.new
m.title = "Reservoir Dogs"
m.year = 1992
m.relevance = 7
m.save
# -------------------------
# Movie 30
m = Movie.new
m.title = 'A Clockwork Orange'
m.year = 1974
m.relevance = 6
m.save
# -------------------------
# Movie 31
m = Movie.new
m.title = "2001: A Space Odyssey"
m.year = 1968
m.relevance = 6
m.save
# -------------------------
# Movie 32
m = Movie.new
m.title = 'Toy Story'
m.year = 1995
m.relevance = 7
m.save
# -------------------------
# Movie 33
m = Movie.new
m.title = "Up"
m.year = 2009
m.relevance = 7
m.save
# -------------------------
# Movie 34
m = Movie.new
m.title = 'Gone Girl'
m.year = 2014
m.relevance = 7
m.save
# -------------------------
# Movie 35
m = Movie.new
m.title = "How to Train your Dragon"
m.year = 2010
m.relevance = 5
m.save
# -------------------------
# Movie 36
m = Movie.new
m.title = 'Jurassic Park'
m.year = 1993
m.relevance = 9
m.save
# -------------------------
# Movie 37
m = Movie.new
m.title = "Into the Wild"
m.year = 2007
m.relevance = 7
m.save
# -------------------------
# Movie 38
m = Movie.new
m.title = 'Catch me if you can'
m.year = 2002
m.relevance = 7
m.save
# -------------------------
# Movie 39
m = Movie.new
m.title = "Monsters, Inc"
m.year = 2001
m.relevance = 6
m.save
# -------------------------
# Movie 40
m = Movie.new
m.title = 'Guardians of the Galaxy'
m.year = 2014
m.relevance = 7
m.save
# -------------------------
# Movie 41
m = Movie.new
m.title = "Furious 7"
m.year = 2015
m.relevance = 6
m.save
# -------------------------
# Movie 42
m = Movie.new
m.title = 'Frozen'
m.year = 2013
m.relevance = 8
m.save
# -------------------------
# Movie 43
m = Movie.new
m.title = "A Star is Born"
m.year = 2018
m.relevance = 6
m.save
# -------------------------
# Movie 44
m = Movie.new
m.title = 'E.T. The Extra-Terrestrial'
m.year = 1982
m.relevance =9
m.save
# -------------------------
# Movie 45
m = Movie.new
m.title = "Star Wars Episode IV: A New Hope"
m.year = 1977
m.relevance = 10
m.save
# -------------------------
# Movie 46
m = Movie.new
m.title = 'Snow White and the Seven Dwarfs'
m.year = 1937
m.relevance = 8
m.save
# -------------------------
# Movie 47
m = Movie.new
m.title = "Terminator 2: Judgment Day"
m.year = 1991
m.relevance = 8
m.save
# -------------------------
# Movie 49
m = Movie.new
m.title = "Harry Potter and the Sorcerer's Stone"
m.year = 2001
m.relevance = 10
m.save
# -------------------------
# Movie 50
m = Movie.new
m.title = "Jaws"
m.year = 1975
m.relevance = 9
m.save
# -------------------------
# Movie 51
m = Movie.new
m.title = "Shrek"
m.year = 2001
m.relevance = 9
m.save
# -------------------------
# Movie 52
m = Movie.new
m.title = "Transformers"
m.year = 2007
m.relevance = 8
m.save
# -------------------------
# Movie 53
m = Movie.new
m.title = "Gladiator"
m.year = 2000
m.relevance = 8
m.save
# -------------------------
# Movie 54
m = Movie.new
m.title = "Home Alone"
m.year = 1990
m.relevance = 8
m.save
# -------------------------
# Movie 55
m = Movie.new
m.title = "Mary Poppins"
m.year = 1964
m.relevance = 8
m.save
# -------------------------
# Movie 56
m = Movie.new
m.title = "Die Hard"
m.year = 1988
m.relevance = 7
m.save
# -------------------------
# Movie 57
m = Movie.new
m.title = "Ghostbusters"
m.year = 1984
m.relevance = 7
m.save
# -------------------------
# Movie 58
m = Movie.new
m.title = "Top Gun"
m.year = 1986
m.relevance = 6
m.save
# -------------------------
# Movie 59
m = Movie.new
m.title = "Finding Nemo"
m.year = 2003
m.relevance = 7
m.save
# -------------------------
# Movie 60
m = Movie.new
m.title = "Rocky"
m.year = 1976
m.relevance = 8
m.save
# -------------------------
# Movie 61
m = Movie.new
m.title = "Casablanca"
m.year = 1942
m.relevance = 6
m.save
# -------------------------
# Movie 62
m = Movie.new
m.title = "Mean Girls"
m.year = 2004
m.relevance = 7
m.save
# -------------------------
# Movie 63
m = Movie.new
m.title = "Harry Potter and the Goblet of Fire"
m.year = 2005
m.relevance = 6
m.save
# -------------------------
# Movie 64
m = Movie.new
m.title = "Misson: Impossible II"
m.year = 2000
m.relevance = 7
m.save
# -------------------------
# Movie 65
m = Movie.new
m.title = "Star Wars Episode I: The Phantom Menace"
m.year = 1999
m.relevance = 7
m.save
# -------------------------
# Movie 66
m = Movie.new
m.title = "Independence Day"
m.year = 1996
m.relevance = 6
m.save
# -------------------------
# Movie 67
m = Movie.new
m.title = "Aladdin"
m.year = 1992
m.relevance = 8
m.save
# -------------------------
# Movie 68
m = Movie.new
m.title = "Indiana Jones and the Last Crusade"
m.year = 1989
m.relevance = 7
m.save
# -------------------------
# Movie 69
m = Movie.new
m.title = "Star Wars Episode V: The Empire Strikes Back"
m.year = 1980
m.relevance = 8
m.save
# -------------------------
# Movie 70
m = Movie.new
m.title = "Grease"
m.year = 1978
m.relevance = 7
m.save
# -------------------------
# Movie 71
m = Movie.new
m.title = "The Expendables"
m.year = 2010
m.relevance = 7
m.save
# -------------------------
# Movie 72
m = Movie.new
m.title = "The Intouchables"
m.year = 2011
m.relevance = 7
m.save
# -------------------------
# Movie 74
m = Movie.new
m.title = "Inside Out"
m.year = 2015
m.relevance = 7
m.save
# -------------------------
# Movie 75
m = Movie.new
m.title = "Twilight"
m.year = 2008
m.relevance = 8
m.save
# -------------------------
# Movie 77
m = Movie.new
m.title = "Deadpool"
m.year = 2016
m.relevance = 7
m.save
# -------------------------
# Movie 78
m = Movie.new
m.title = "Jumanji"
m.year = 1995
m.relevance = 8
m.save
# -------------------------
# Movie 79
m = Movie.new
m.title = "Spider-man"
m.year = 2002
m.relevance = 8
m.save
# -------------------------
# Movie 81
m = Movie.new
m.title = "Skyfall"
m.year = 2012
m.relevance = 6
m.save
# -------------------------
# Movie 82
m = Movie.new
m.title = "Casino Royale"
m.year = 2006
m.relevance = 6
m.save
# -------------------------
# Movie 83
m = Movie.new
m.title = "GoldenEye"
m.year = 1995
m.relevance = 7
m.save
# -------------------------
# Movie 84
m = Movie.new
m.title = "The Fast and the Furious"
m.year = 2001
m.relevance = 7
m.save
# -------------------------
# Movie 85
m = Movie.new
m.title = "Rambo"
m.year = 2008
m.relevance = 7
m.save

p "Populated #{Movie.all.length} movies"







































